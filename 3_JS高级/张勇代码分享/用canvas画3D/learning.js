var canvas = document.getElementById("cas");
    ...
    vpy = canvas.height/2;
    Radius = 150;                       //
    LayerBallNum = 360 / 30;        // 横向圆周个数
    LayerIntervalUp = 360 / 30;     // 纵向圆周个数
    balls = [];                     //创建数组保存 球
    angleX = Math.PI/100;           //X方向度数
    angleY = Math.PI/100;           //Y方向度数
...
var layer = function (num, up) {
    this.radius = Math.sqrt(Math.pow(Radius,2) - Math.pow(Radius * Math.cos(num * Math.PI * 2 / LayerBallNum), 2));  //设定不同经度上的圆周半径 this.x = 0;著作权归作者所有。
    this.x = 0;
    this.y = 0;
    this.up = up;
}
